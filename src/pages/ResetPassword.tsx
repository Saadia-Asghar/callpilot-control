...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    const hasRecovery = window.location.hash.includes("access_token") || searchParams.get("type") === "recovery";
    if (hasRecovery) {
      setPageState("form");
    } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4 relative">
      <Button variant="ghost" size="sm" className="absolute top-4 left-4 gap-1.5 text-muted-foreground" onClick={() => navigate("/auth")}>
          ...
          {pageState === "expired" && (
            <div className="text-center space-y-4 py-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">This password reset link is invalid or has expired.</p>
              </div>
              <Button className="gradient-primary text-primary-foreground border-0" onClick={() => navigate("/forgot-password")}>Request New Link</Button>
            </div>
          )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-10" autoComplete="new-password" autoFocus />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                  <Input id="confirmPassword" type="password" placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-10" autoComplete="new-password" />
                </div>
              </div>
              ...
              <Button type="submit" className="w-full gradient-primary text-primary-foreground border-0" disabled={submitting || password.length < 6 || password !== confirmPassword}>
          ...
          {pageState === "success" && (
            <div className="text-center space-y-4 py-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">You can now log in with your new password.</p>
              </div>
              <Button className="gradient-primary text-primary-foreground border-0" onClick={() => navigate("/auth")}>Go to Login</Button>
            </div>
          )}
        </div>
      </div>
